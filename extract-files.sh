...
function blob_fixup() {
    ...
    vendor/lib/hw/camera.msm8996.so)
        ...
        "${PATCHELF_0_18}" --replace-needed "libcamera_client.so" "libcamera_metadata_helper.so" "${2}"
        "${PATCHELF_0_18}" --replace-needed "libandroid.so" "libshims_android.so" "${2}"
        ;;
    vendor/lib/libarcsoft_beauty_shot.so|vendor/lib/libmmcamera2_stats_modules.so‎|vendor/lib/libmpbase.so)
        "${PATCHELF_0_18}" --replace-needed "libandroid.so" "libshims_android.so" "${2}"
        ;;
